...
class ResScore(nn.Module):
    def __init__(self, config):
        ...
        self.convs = nn.Sequential(
            nn.Conv2d(input_channels, self.nef, 3, 1, 1),
        ...
        self.deconvs = nn.Sequential(
            ...
            nn.Conv2d(self.ndf, config.data.out_channels, 3, 1, 1)
        )

    def forward(self, x):
    # Suppose x has shape [B, C+1, H, W] if noise_conditioning is on
    # with the last channel being sigma.
        img, sigma_map = x[:, :-1], x[:, -1:]
        img = 2*img - 1.    # only shift the image
        x = torch.cat([img, sigma_map], dim=1)
...
class UNetScore(nn.Module):
    def __init__(self, config):
        ...
        if config.data.image_size == 32:
            ...
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None,
                                         norm_layer=norm_layer, innermost=True)
        else:  # Default case for other sizes (including 80)
    # Use a similar structure as the 32x32 case but with more downsampling
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=None,
                                     norm_layer=norm_layer, innermost=True)
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block,
                                         norm_layer=norm_layer)
    # Add more downsampling blocks if needed for larger images
            unet_block = UnetSkipConnectionBlock(ngf * 8, ngf * 8, input_nc=None, submodule=unet_block,
                                         norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBlock(ngf * 4, ngf * 8, input_nc=None, submodule=unet_block,
                                             norm_layer=norm_layer)
